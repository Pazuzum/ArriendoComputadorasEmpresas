            ...
            <nav className="flex space-x-12 text-white text-lg font-bold">
                {isAuthenticated ? (
                        <>
                            ...
                            {isAdmin && (
                                <Link to="/admin" className="py-2 hover:text-blue-600 transition-colors duration-200 ml-4">Panel Admin</Link>
                            )}
                            ...
                            {carritoVisible && (
                                <div className="absolute top-[70px] right-2 bg-white w-[400px] rounded-[20px] z-[1]">
                                    {items.length ? (
                                        <>
                                            <div className="">
                                                <h2 className="text-center text-[20px] font-bold py-5 border-b border-black">Cotización</h2>
                                            </div>
                                            <div className="flex justify-center items-center py-5 gap-5">
                                                <h3 className="text-[15px] font-bold">Total:</h3>
                                                <span className="text-[20px] font-bold">${items.reduce((a,b)=> a + (b.precio||0)*(b.qty||1), 0)}</span>
                                                <button className="bg-black text-white p-1 rounded-[15%]" onClick={()=>{ /* could route to /cotizar */ }} >Ver</button>
                                            </div>
                                            <div className="flex justify-center items-center py-5 gap-5 border-t border-black">
                                                <button className="w-1/4 h-11 bg-[#afc] rounded-lg border-none font-bold text-lg hover:bg-[#bdc] cursor-pointer" onClick={()=> window.location.href='/cotizar'} >Ir a cotización</button>
                                            </div>
                                        </>
                                    ) : (
                                        <p className="p-5 text-[17px] text-[#666] text-center">La cotización está vacía.</p>
                                    )}
                                </div>
                            )}
                        </>

...
export const listarCotizaciones = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Error al listar cotizaciones', error });
  }
};
...
export const cambiarEstado = async (req, res) => {
  try {
    ...
    // Si se acepta, validar disponibilidad y decrementar
    if (estado === 'ACEPTADA') {
      // Verificar stock
      for (const it of cot.items) {
        if (!it.productId) continue; // si no hay referencia, asumimos no controlado
